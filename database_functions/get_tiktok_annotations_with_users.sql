-- ============================================================================
-- Function: get_tiktok_annotations_with_users
-- ============================================================================
-- Returns map annotations for TikTok/external places with grid-based clustering
...
CREATE OR REPLACE FUNCTION public.get_tiktok_annotations_with_users(
    p_user_id text,
...
BEGIN
    ...
    RETURN QUERY
    WITH tiktok_annotations AS (
        SELECT
            ep.place_id AS id,
            p.name,
            p.location AS coordinate,
            ARRAY[ep.user_id] AS user_ids,
            COALESCE(
        ...
        INNER JOIN places p ON ep.place_id = p.id
