...
class PlaceService: ObservableObject {
    ...
    func fetchPlacesInBounds(
        northEast: CLLocationCoordinate2D,
        southWest: CLLocationCoordinate2D,
        completion: @escaping ([DetailPlace]?, Error?) -> Void
    ) {
        print("🗺️ Fetching places in bounds: NE(\(northEast.latitude), \(northEast.longitude)) SW(\(southWest.latitude), \(southWest.longitude))")
        
        // Create a GeoPoint query for the bounds
        ...
        query.getDocuments { (snapshot, error) in
            if let error = error {
                print("❌ Error fetching places in bounds: \(error.localizedDescription)")
                completion(nil, error)
                return
            }
            
            guard let documents = snapshot?.documents else {
                print("📭 No places found in bounds")
                completion([], nil)
                return
            }
            
            let places = documents.compactMap { try? $0.data(as: DetailPlace.self) }
            print("✅ Fetched \(places.count) places in bounds")
            completion(places, nil)
        }
    }
    
    /// Async version of bounds-based loading
    func fetchPlacesInBounds(
        northEast: CLLocationCoordinate2D,
        southWest: CLLocationCoordinate2D
    ) async throws -> [DetailPlace] {
        return try await withCheckedThrowingContinuation { continuation in
            fetchPlacesInBounds(northEast: northEast, southWest: southWest) { places, error in
                if let error = error {
    ...
    func fetchUserPlacesInBounds(
        userId: String,
        northEast: CLLocationCoordinate2D,
        southWest: CLLocationCoordinate2D,
        completion: @escaping ([DetailPlace]?, Error?) -> Void
    ) {
        print("👤 Fetching user places in bounds for user: \(userId)")
        ...
        query.getDocuments { (snapshot, error) in
            if let error = error {
                print("❌ Error fetching user places in bounds: \(error.localizedDescription)")
                completion(nil, error)
                return
            }
            
            guard let documents = snapshot?.documents else {
                print("📭 No user places found in bounds")
                completion([], nil)
                return
            }
            
            let places = documents.compactMap { try? $0.data(as: DetailPlace.self) }
            print("✅ Fetched \(places.count) user places in bounds")
            completion(places, nil)
        }
    }
    
    /// Load user's lists within bounds
    func fetchUserListsInBounds(
        userId: String,
        northEast: CLLocationCoordinate2D,
        southWest: CLLocationCoordinate2D,
        completion: @escaping ([PlaceList]?, Error?) -> Void
    ) {
        print("📋 Fetching user lists in bounds for user: \(userId)")
        
        // First get all user lists
        fetchLists(userId: userId) { lists in
            // Then filter places within bounds for each list
            var filteredLists: [PlaceList] = []
            
            for list in lists {
                let placesInBounds = list.places.filter { place in
                ...
                if !placesInBounds.isEmpty {
                    let filteredList = PlaceList(
                        id: list.id,
                        name: list.name,
                    ...
                    filteredLists.append(filteredList)
                }
            }
            
            print("✅ Fetched \(filteredLists.count) user lists with places in bounds")
            completion(filteredLists, nil)
        }
    }

    // MARK: - Existing Methods (Keep for backward compatibility)
