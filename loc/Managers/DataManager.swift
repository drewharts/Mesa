...
class DataManager {
    ...
    func loadUserFavoritePlaces(userId: String, forUser: User? = nil) {
    ...
    func loadUserPlaceLists(userId: String, forUser: User? = nil) {
    ...
    func loadFollowers(userId: String) {
        fireStoreService.fetchFollowerProfiles(for: userId) { profiles, error in
            if let error = error {
                print("Error loading following profiles: \(error.localizedDescription)")
                self.profileViewModel.finishLoading()
                return
